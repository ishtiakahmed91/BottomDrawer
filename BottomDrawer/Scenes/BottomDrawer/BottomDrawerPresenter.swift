...
extension BottomDrawerPresenter: BottomDrawerPresentationLogic {
    func presentItems(response: BottomDrawer.LoadItems.Response) {
        DispatchQueue.main.async {[weak self] in
            ...
            let viewModel = BottomDrawer.LoadItems.ViewModel(items: response.items, drawerPosition: response.drawerPosition, cellType: response.cellType, selectedRow: selectedRow)
